...
.claude/
  commands/              # Slash commands (user-invoked): /draft /hook /visuals /package /publish (/draft = cold Opus subagents: writer → section/arc ensemble → fixer → ściskacz; /publish defaults to Agent-Teams, auto-fallback in-session)
...
/draft <slug>                              # Agent 3 — pisarz → ensemble (section+arc) → fixer → ściskacz (zimne subagenty Opus, jeden przebieg, no API)
...
| 3 (whole chain) | `/draft <slug>` (Claude Code slash command; pisarz → ensemble section+arc → fixer → ściskacz, cold Opus subagents, one pass, no API) | Opus 4.8 (in-session lead + cold subagent contexts — no API) | `02_verified_research.md` | `md/03a_draft.md`, `md/03b_corrections.md`, `md/04_final_presqueeze.md` + `md/04_final.md` |
...
| 3b section-checker (×N, per `## `) | (inside `/draft`) cold subagent; sentence `[Z]` + context `[K]`; `03b_section_checker.md` | Opus 4.8 (cold subagent — no API) | frozen `md/03a_draft.md` | `md/iter/sek_NN.md` |
...
| 3d ściskacz | (inside `/draft`) cold subagent, **cut-only**; removes 6 over-writing modes; `03d_compressor.md` | Opus 4.8 (cold subagent — no API) | `md/04_final.md` (fixer) | `md/04_final.md` (lean) + `md/04_final_presqueeze.md` |
