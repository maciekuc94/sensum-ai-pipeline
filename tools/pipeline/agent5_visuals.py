...
only the deterministic work:
  ...
  --expand          Inject CHARACTER_DESCRIPTION + STYLE_SUFFIX constants into the
                    compact **Visual:** fields of 05_image_prompts.md, rebuild phrase files.
...
def _expand_mode(slug: str) -> None:
    """Expand **Visual:** fields in compact 05_image_prompts.md into full **Imagen prompt:** entries.
...
def main() -> None:
    ...
    parser.add_argument(
        "--expand",
        action="store_true",
        help="Expand **Visual:** fields in existing 05_image_prompts.md into full Imagen prompts, then rebuild phrase files.",
    )
    ...
    if prompts_path.exists():
        ...
        if "**Visual:**" in existing_content:
            print("Found compact 05_image_prompts.md (generated by /visuals). Running --expand...")
            _expand_mode(slug)
            return
        elif "**Imagen prompt:**" in existing_content:
            print("05_image_prompts.md already expanded. Nothing to do.")
            print(f"Review {OUTPUT_FILENAME} and run Agent 6 when ready:")
            print(f'  PYTHONIOENCODING=utf-8 python tools/pipeline/agent6_images.py "{slug}" --generate')
            sys.exit(0)

    print("\nError: md/05_image_prompts.md not found.")
