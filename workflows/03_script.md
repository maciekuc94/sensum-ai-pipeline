...
| 3n | Dedupe | `md/03a_draft.md` + `outputs/*/md/06_script_narration.md` (corpus) | revised `md/03a_draft.md` + `md/03_novelty_report.md` (and `md/03a_draft.bak.md` on first run) |
...
- `outputs/[slug]/md/02_verified_research.md` must exist (Agent 2 output)
- `ANTHROPIC_API_KEY` set in `.env`
- `workflows/style_guide.md` and `workflows/narrative_architectures.md` present
...
The four architectures (defined in `workflows/narrative_architectures.md`):
...
```bash
python tools/agent3n_novelty.py "emotional-dysregulation-in-adhd"
```

Catches reuse of phrases, metaphors, and structural beats from prior shipped scripts. Runs three passes:

- **Pass A — literal 4-gram match.** Deterministic Python. Tokenizes the draft and every prior `outputs/*/md/06_script_narration.md`, builds a 4-token n-gram index, flags every span the draft shares with the corpus. Skips n-grams composed entirely of stopwords.
...
| Error | Fix |
|-------|-----|
| `md/02_verified_research.md not found` | Run Agent 2: `python tools/agent2_verify.py "{slug}"` |
