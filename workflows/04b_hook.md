...
1. **Agent 4a must have run successfully.** The file `outputs/[slug]/md/04_script_final.md` must exist.
...
```bash
python tools/agent4b_hook.py "why-you-can-t-just-relax-on-weekends"
```
...
| File | Behavior |
|---|---|
| `outputs/[slug]/md/04_script_final.md` | **Modified in place** — opening replaced if any rewrite was applied. |
| `outputs/[slug]/md/04_script_final.bak.md` | **Created once** on the first refine; never overwritten. |
| `outputs/[slug]/md/04b_hook_score.md` | **Overwritten each run** — full multi-attempt log with both tier breakdowns. |
...
```bash
cp outputs/[slug]/md/04_script_final.bak.md outputs/[slug]/md/04_script_final.md
```
...
- Architecture choices for the hook shape: [workflows/narrative_architectures.md](narrative_architectures.md)
- Prior step (script editing): [workflows/04a_edit.md](04a_edit.md)
...
```bash
python tools/agent5_visuals.py "why-you-can-t-just-relax-on-weekends"
python tools/agent6_narration.py "why-you-can-t-just-relax-on-weekends"
```
