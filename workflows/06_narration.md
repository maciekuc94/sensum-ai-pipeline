...
1. **Agent 4 must have run successfully.** The file `outputs/[slug]/04_script_final.md` must exist.
...
Input : outputs/emotional-dysregulation-in-adhd/04_script_final.md
Output: outputs/emotional-dysregulation-in-adhd/06_script_narration.md
Words : 1,712
...
Open `outputs/[slug]/06_script_narration.md` and confirm:
...
**"Error: outputs/[slug]/04_script_final.md not found"**
...
```bash
python tools/agent7_tts.py "[slug]"       # optional — TTS reference audio
python tools/agent8_publish.py "[slug]"   # titles, hooks, Shorts, metadata
```
