...
## Inputs
- `outputs/{slug}/md/04_script_final.md` — Script for concept generation
- `outputs/{slug}/md/07_publish_package.md` — Title, hook, and metadata context

## Command

**Standard (recommended — no grain):**
```bash
PYTHONIOENCODING=utf-8 python tools/agent10_thumbnails.py "<slug>" --no-grain
```
...
## Outputs
- `outputs/{slug}/thumbnails_no_grain/thumbnail_01.png` … `thumbnail_05.png` (use these)
- `outputs/{slug}/thumbnail_prompts.md` — Saved prompts for reference / `--reuse-prompts`
