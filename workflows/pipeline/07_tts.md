...
> **Currently blocked.** `tools/pipeline/agent7_tts.py` sets `MODEL_VERIFIED = False` because `GEMINI_TTS_MODEL = "gemini-3.1-flash-tts-preview"` was never confirmed against the Vertex AI catalog. Any invocation exits with code 1. Verify the correct preview model name (Vertex console or `gcloud ai models list`), update `GEMINI_TTS_MODEL` in the script, and flip `MODEL_VERIFIED = True` before next run.
...
```bash
python tools/pipeline/agent7_tts.py what-porn-does-to-your-brain
```
...
| Gemini TTS fails with model not found | Update `GEMINI_TTS_MODEL` in `tools/pipeline/agent7_tts.py` |
